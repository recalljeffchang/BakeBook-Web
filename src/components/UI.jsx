...
export function RecipeCardRow({ recipe, onClick, highlightQuery }) {
  const catColors = {
    '麵包': '#FEF3E7', '蛋糕': '#FFF0F5',
    ...
    <div className="recipe-card-row" onClick={onClick} style={{ cursor: 'pointer' }}>
      ...
      <div className="recipe-card-info">
        <div className="recipe-card-name">
          <HighlightText text={recipe.name} query={highlightQuery} />
        </div>
        {highlightQuery && recipe.subtitle && (
          <div style={{ fontSize: 12, color: '#888', marginTop: -2 }}>
            <HighlightText text={recipe.subtitle} query={highlightQuery} />
          </div>
        )}
        ...
        <div className="recipe-card-tags">
          ...
          {matchedIngredients.length > 0 && (
            <TagBadge
              text={`🧂 ${matchedIngredients.slice(0, 2).join('、')}${matchedIngredients.length > 2 ? '…' : ''}`}
              bg="#F0FFF4"
              color="#2E7D52"
            />
          )}
...
export function QuantityStepper({ value, onChange, min = 1 }) {
  return (
    <div className="qty-stepper">
      ...
      <button className="qty-btn" onClick={() => onChange(value + 1)} type="button">＋</button>
    </div>
  );
}
