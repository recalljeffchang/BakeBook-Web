...
export default function Timer({ initialMinutes = 35, onClose }) {
  ...
  // Keep ref in sync with latest remaining value
  ...
  const addMinutes = (m) => {
    ...
    setTotalSec(prev => Math.max(prev, remainingRef.current + add));
  };
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 28 }}>
        <div style={{ position: 'relative', width: 224, height: 224 }}>
          <svg width="224" height="224" className="timer-ring-svg" style={{ position: 'absolute', top: 0, left: 0 }}>
            <circle cx="112" cy="112" r={radius} fill="none" stroke="rgba(255,255,255,0.1)" strokeWidth="8" transform="rotate(-90 112 112)" />
            <circle
