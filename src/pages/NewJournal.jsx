...
export default function NewJournalModal({ onClose, initialRecipe }) {
  ...
  const hasOvenStep = selectedRecipe?.steps?.some(s => s.description?.includes('烤箱')) ?? false;
  // 預填烤溫與時間（若食譜有記錄）
  const suggestedTemp = selectedRecipe?.ovenTemp || '';
  const suggestedTime = selectedRecipe?.bakeMinutes || '';
  ...
  const save = () => {
    const entry = {
      ...
      totalWeightGrams: toGrams(totalWeight, weightUnit),
      materialCost: Number(materialCost) || 0,
      saleEnabled,
      soldQuantity: Number(soldQuantity) || 0,
      unitPrice: Number(unitPrice) || 0,
      saleNote,
      ovenTemp: ovenTemp || null,
      bakeMinutes: Number(bakeMinutes) || null,
    };
      ...
      <div className="modal-sheet" onClick={e => e.stopPropagation()}>
        ...
        <div style={{ padding: '16px 18px 80px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div className="card">
            <FormField label="食譜名稱" color="#9E7B55">
              <select
                ...
                onChange={e => {
                  ...
                  if (selected) {
                    setRecipeId(selected.id);
                    setRecipeName(selected.name);
                    // 自動預填烤箱資訊
                    if (selected.ovenTemp) setOvenTemp(selected.ovenTemp);
                    if (selected.bakeMinutes) setBakeMinutes(String(selected.bakeMinutes));
                  } else {
                    setRecipeId('');
                    setRecipeName('');
                  }
            ...
            {!recipeId && (
              <FormField label="或手動輸入名稱" color="#9E7B55">
                <input className="bake-input" placeholder="輸入食譜名稱" value={recipeName} onChange={e => setRecipeName(e.target.value)} />
              </FormField>
            )}
            <div style={{ height: 10 }} />
          ...
          {/* Oven info — shown when recipe has oven step */}
          {hasOvenStep && (
            <div className="card" style={{ background: '#FEF3E7', border: '1px solid #F0D9B5' }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: '#9E6A2A', letterSpacing: 0.4, textTransform: 'uppercase', marginBottom: 10 }}>
                🔥 烤箱資訊
              </div>
              {suggestedTemp && (
                <div style={{ fontSize: 12, color: '#B08A5A', marginBottom: 8 }}>食譜建議：{suggestedTemp}，烘烤 {suggestedTime} 分鐘</div>
              )}
              <div style={{ display: 'flex', gap: 8 }}>
                <FormField label="烤溫（例：180°C）" color="#9E6A2A">
                  <input className="bake-input" placeholder="例：180°C" value={ovenTemp} onChange={e => setOvenTemp(e.target.value)} />
                </FormField>
                <FormField label="烘烤時間（分鐘）" color="#9E6A2A">
                  <input className="bake-input" type="number" placeholder="例：30" value={bakeMinutes} onChange={e => setBakeMinutes(e.target.value)} />
                </FormField>
              </div>
            </div>
          )}
