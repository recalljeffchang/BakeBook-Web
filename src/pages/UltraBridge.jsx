...
export default function UltraBridge({ onBack }) {
  ...
  // Step 4 (JSON paste) is always accessible — user might skip image steps
  const activeStep = saved ? 6 : parsed ? 5 : jsonRaw.trim() ? 4 : promptCopied ? 3 : imageFile ? 2 : 1;
    ...
    <div className="page" style={{ background: '#F2F0EB' }}>
      ...
      <div style={{ padding: '14px 16px 40px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{
          ...
          borderRadius: 16,
          padding: 16,
          border: `2px solid ${parsed ? '#2E7D52' : jsonRaw.trim() ? '#B8860B' : '#f0ece6'}`,
          transition: 'border-color 0.2s',
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
            <StepBadge n={4} done={parsed !== null} active={!parsed && jsonRaw.trim().length > 0} />
            <div>
